...
void reference_random_forest(ARGS HERE);
...
int main(int argc, char** argv) {
    ...
    printf("\n");
    //PRINT something about data set here
    ...
    if (thread_count <= -1)
    {
        ...
        solution sol1;
        sol1.prediction = (int*)malloc(sizeof(int) * g->num_nodes);
        solution sol2;
        sol2.prediction = (int*)malloc(sizeof(int) * g->num_nodes);

        double parallel_base;
        ...
        for (int i = 0; i < n_usage; i++)
        {
            ...
            start = CycleTimer::currentSeconds();
            reference_random_forest(g, &sol4);
            ...
            sprintf(buf, "%4d:   %.4f (%.4fx)\n",
                    num_threads[i], top_time, top_base/top_time);
